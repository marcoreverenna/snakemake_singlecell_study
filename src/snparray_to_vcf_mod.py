...
def load_manifest(manifest_path='data/raw/HumanCytoSNP-12v2-1_hg19.bpm'):
    """ Generate dictionary of variants from Illumina manifest file

    Args:
        manifest_path (str, optional): _description_. Defaults to 'data/raw/HumanCytoSNP-12v2-1_hg19.bpm'.
...
def import_vcf_reference():
    ...
    """
    return allel.vcf_to_dataframe('data/raw/GM12878_gDNA_3.vcf.gz')
...
if __name__=='__main__':
    ...
    # load manifest
    to_refalt = load_manifest('data/raw/HumanCytoSNP-12v2-1_hg19.bpm')

    # load parquet file
    # use params['parquet'] if loading from command line argument
    df = pd.read_parquet('data/raw/PGD036_stage2_nofilter_hg19.df.parquet',engine='pyarrow')
